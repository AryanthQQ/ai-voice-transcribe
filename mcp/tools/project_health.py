...
LARGE_FILE_THRESHOLD_BYTES = 500 * 1024  # 500 KB
SKIP_DIRS = {".git", "__pycache__", "node_modules", "venv", ".venv", ".next", "dist", "build"}
ENTRY_POINTS = {"main.py", "app.py", "worker.py", "run.py", "manage.py", "setup.py"}

def _should_skip(path: Path) -> bool:
    return any(part in SKIP_DIRS for part in path.parts)

def _get_internal_module_path(base_dir: Path, module_name: str) -> Optional[Path]:
...
def handle(project_root: Optional[str] = None) -> dict:
    ...
    # Check for Duplicate Logic (Files with the exact same name in different top-level backend dirs)
    # This specifically catches the `backend/app.py` vs `python_backend/app.py` issue.
                ...
                if isinstance(node, ast.Import):
                    ...
                    if node.module:
                        ...
                        if "backend" in py_file.parts and base_module in internal_namespaces:
                            # Attempt to resolve the path relative to the backend directory
                            backend_dir = root / "backend"
                            resolved = _get_internal_module_path(backend_dir, node.module)
                            if not resolved:
    ...
    for py_file in py_files:
        if "backend" in py_file.parts and "mcp" not in py_file.parts:
            # If the file's stem is not an imported module and not an entry point
            stem = py_file.stem
            if stem not in ENTRY_POINTS and stem != "__init__":
                if stem not in imported_modules:
                    # Check if it's an API route (often dynamically loaded, so maybe used)
